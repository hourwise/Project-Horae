...
| Selected session runtime is unavailable       | Implemented and tested at session level       | Not decided; `assessState()` reports only                                     | Session composition availability                                                              | Derived `degraded` assessment with runtime IDs              | No                                              | Treating the session as ready or silently replanning it              |
...
| Incompatible protocol version                 | Implemented and tested                        | No                                                                            | Runtime compatibility                                                                         | `RuntimeProtocolCompatibilityError` before session creation | No                                              | Silent fallback to an incompatible runtime                           |
...
- session state is assessed from the selected composition without changing runtime lifecycle state or creating a replacement composition.
